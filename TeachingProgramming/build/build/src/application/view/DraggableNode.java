...
public class DraggableNode extends AnchorPane {
	...
	//LIST IMPLEMENTATION
	private boolean Port1open = true, Port2open = true;
	private boolean Socket1open = true, Socket2open = true;

	//Keep a list of all link ids that are connected to this node.
	private final List mLinkIds = new ArrayList();

	//NEW IMPLEMENTATION
	//private final List<Link> LinkConnections = new ArrayList();
	...
	public void registerPortLink(DraggableNode target) {

		if (Port1open) {
			comp.registerPortLink(target.comp);
			Port1open = false;
	...
	public void registerSocketLink(DraggableNode source) {
	...
	public void freePortLink(int portNum) {
		if (portNum == 1) {
			Port1open = true;
			comp.removePortLink(1);
		}
		if (portNum == 2) {
			Port2open = true;
			comp.removePortLink(2);
		}
	}

	public void freeSocketLink(int socketNum) {
		if (socketNum == 1) {
			Socket1open = true;
			comp.removeSocketLink(1);
		}
		if (socketNum == 2) {
			Socket2open = true;
			comp.removeSocketLink(1);
